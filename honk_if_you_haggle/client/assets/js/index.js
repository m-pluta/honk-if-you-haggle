...
async function listCars() {
  ...
  if (data) {
    ...
    for (const key in data) {
      ...
      copyHTML.querySelector('.card-car-title').textContent =
        carData.make + ' ' + carData.model;
      copyHTML.querySelector(
        '.card-car-year'
      ).innerHTML = `<strong>Year: </strong> ${carData.year}`;
      copyHTML.querySelector(
        '.card-car-mileage'
      ).innerHTML = `<strong>Mileage: </strong> ${carData.mileage}`;
      copyHTML.querySelector(
        '.card-car-price'
      ).textContent = `£${carData.price}`;
...
function attachValidationListeners() {
  ...
  formMake.addEventListener('input', (event) => {
    changeValidity(
      formMake,
      regexLettersWhitespace.test(formMake.value.trim())
    );
  });
  formModel.addEventListener('input', (event) => {
    changeValidity(
      formModel,
      regexLettersWhitespaceNumbers.test(formModel.value.trim())
    );
  });
  ...
  formColour.addEventListener('input', (event) => {
    changeValidity(
      formColour,
      regexLettersWhitespace.test(formColour.value.trim())
    );
  });
